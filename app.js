...
var Flat_polygon = function () {
	...
	if (!gl.getShaderParameter(vertexShader1, gl.COMPILE_STATUS)) {
		console.error('ERROR canot compile vertex shader!', gl.getShaderInfoLog(vertexShader1));
		return;
	}

	gl.compileShader(fragmentShader1);
	if (!gl.getShaderParameter(fragmentShader1, gl.COMPILE_STATUS)) {
		console.error('ERROR cannot compile fragment shader!', gl.getShaderInfoLog(fragmentShader1));
		return;
	}

	var program = gl.createProgram();
	gl.attachShader(program, vertexShader1);
	gl.attachShader(program, fragmentShader1);
